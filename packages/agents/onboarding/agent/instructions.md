...
- Aim to import 25 to 50 high-quality tweet/blog/LinkedIn references per organization when sources permit, using `add_references`; fewer is correct when the company has less owned material or when the remaining candidates are one-liners or otherwise weak.
