...
import type { SocialCallbackOutcome } from "@/types/auth";
...
const exchangeSocialCode = Effect.fn("auth.social.exchangeCode")(function* (
  ...
  yield* Effect.tryPromise({
    try: () =>
      saveSession(
        response,
        process.env.CONSOLE_APP_URL ?? "http://localhost:3003"
      ),
    catch: (cause) =>
...
export async function GET(request: NextRequest) {
  ...
  const returnTo = sanitizeReturnTo(stateReturnTo) ?? "/dashboard";
