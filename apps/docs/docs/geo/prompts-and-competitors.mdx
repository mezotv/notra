...
Each prompt is written the way a person types into ChatGPT: lowercase, one intent, no trailing question mark. A custom prompt must be between 8 and 300 characters. Suggested prompts that contain your company name or an alias are dropped during onboarding, because a branded question tells you nothing about unaided recall.
...
<img
  src="/images/geo/prompts-light.png"
  alt="Notra GEO prompts showing tracked questions and brand presence"
  class="doc-image-light doc-screenshot"
/>
<img
  src="/images/geo/prompts-dark.png"
  alt="Notra GEO prompts showing tracked questions and brand presence"
...
| --- | --- | --- |
| `rows` | `array` | 1 to 500 objects with `prompt` (8 to 300 characters, required) and `enabled` (boolean, optional). |
| `csv` | `string` | Raw CSV text, up to 1 MiB, with a `prompt` column and an optional `enabled` column. Used when `rows` is omitted. |
...
| Field | Type | Description |
| --- | --- | --- |
| `checks` | `integer` | Recorded answers across every engine that responded. |
| `mentions` | `integer` | How many of those answers mentioned the tracked brand. |
| `engines` | `string[]` | Engines the conversation was played against. |
...
<img
  src="/images/geo/competitors-light.png"
  alt="Notra GEO competitors showing tracked brands and share of voice"
  class="doc-image-light doc-screenshot"
/>
<img
  src="/images/geo/competitors-dark.png"
  alt="Notra GEO competitors showing tracked brands and share of voice"
  class="doc-image-dark doc-screenshot"
/>
...
| Field | Type | Description |
| --- | --- | --- |
| `name` | `string` | **Required.** 1 to 128 characters. |
| `previousName` | `string` | Set to rename an existing competitor. |
| `domain` | `string \| null` | **Required.** Website domain, up to 128 characters, or `null`. |
| `synonyms` | `string[]` | Up to 8 entries. |
| `kind` | `'direct' \| 'indirect'` | Defaults to `direct` when omitted on create. |
| `color` | `string \| null` | Chart color, up to 128 characters. |
...
| Field | Type | Description |
| --- | --- | --- |
| `rows` | `array` | 1 to 25 objects with `name` (required), `domain`, `kind`, and `synonyms` (up to 8). |
| `csv` | `string` | Raw CSV text, up to 1 MiB, with a `name` column and optional `domain`, `kind`, `synonyms` columns. Used when `rows` is omitted. |
