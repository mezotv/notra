...
<img
  src="/images/geo/agent-readiness-light.png"
  alt="Notra Agent Readiness showing a website score and improvement checklist"
  class="doc-image-light doc-screenshot"
/>
<img
  src="/images/geo/agent-readiness-dark.png"
  alt="Notra Agent Readiness showing a website score and improvement checklist"
...
| --- | --- | --- |
| `targetUrl` | `string` | The normalised website URL the reports belong to. |
| `report` | `object \| null` | The latest completed report: `id`, `status`, `targetUrl`, `score`, `scoreLabel`, `scoreBreakdown`, `issues[]`, `eligibleChecks`, `reportUrl`, `errorMessage`, `scannedAt`, `createdAt`. |
| `scan` | `object \| null` | A run newer than the completed report that is still `running` or has `failed`, in the same shape. |
| `history` | `array` | Completed scans, oldest first, each with `id`, `score`, `failedCount`, `partialCount`, `scannedAt`. |
