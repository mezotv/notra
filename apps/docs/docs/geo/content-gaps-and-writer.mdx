...
<img
  src="/images/geo/content-gaps-light.png"
  alt="Notra GEO content gaps showing questions where AI engines recommend competitors"
  class="doc-image-light doc-screenshot"
/>
<img
  src="/images/geo/content-gaps-dark.png"
  alt="Notra GEO content gaps showing questions where AI engines recommend competitors"
  class="doc-image-dark doc-screenshot"
/>
...
| --- | --- | --- |
| `topic` | `string` | **Required.** 3 to 200 characters. Replaced by the source prompt when `sourceKind` is `gap`, `prompt`, or `search_console`. |
| `autoApprove` | `boolean` | **Default:** `false`. Start the writer in the same call instead of leaving the brief in draft. |
| `contentSubtype` | `string` | One of `guide`, `comparison`, `listicle`, `how-to`, `faq`, `alternatives`. |
| `brandVoiceIds` | `string[]` | Up to 8 ids; only the first is used and it overrides the project's brand identity. |
| `competitorIds` | `string[]` | Up to 25 tracked competitor ids to position against. |
| `sitemapId` | `string` | Sitemap whose pages the planner may link to. |
| `sourceKind` | `string` | One of `manual`, `gap`, `prompt`, `search_console`. |
| `sourceId` | `string` | Gap, prompt, or search-console suggestion id. An open brief for the same source is reused instead of planning a new one. |
...
| Field | Type | Description |
| --- | --- | --- |
| `companyName` | `string` | **Required.** 1 to 128 characters. |
| `aliases` | `string[]` | **Required.** Up to 10 entries. |
| `languages` | `string[]` | **Required.** 1 to 4 supported language codes. An unknown code is rejected with `400`. |
| `engines` | `string[]` | **Required.** 1 to 64 model ids from the catalog this organization can see (the ids `GET /geo/settings` returns). An unknown id is rejected with `400`. Engines not visible to the caller keep their stored selection. |
| `enforceZdr` | `boolean` | **Required.** Forced off without the ZDR add-on. |
| `nonZdrApprovedEngines` | `string[]` | **Required.** Up to 64 model ids allowed to run despite lacking ZDR. |
| `enabled` | `boolean` | **Required.** Whether recurring scans run. |
| `scanIntervalHours` | `integer` | **Required.** One of 24, 48, 72, 168, 336, 720. |
