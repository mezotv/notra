...
| --- | --- | --- |
| `limit` | `integer` | **Default:** `10`. Items per page. **Range:** 1 to 100. The default depends on the endpoint (see the table above). |
| `page` | `integer` | **Default:** `1`. The page number to retrieve. Pages start at 1. |
...
Every paginated response includes a `pagination` object with metadata about the current page and navigation options:

<Expandable title="Pagination properties">

| Field | Type | Description |
| --- | --- | --- |
| `limit` | `integer` | The number of items per page (matches your query parameter or the endpoint default). |
| `currentPage` | `integer` | The page number being returned. |
| `nextPage` | `integer \| null` | The next page number, or `null` when there is no next page. |
| `previousPage` | `integer \| null` | The previous page number, or `null` on page 1. |
| `totalPages` | `integer` | The total number of pages. Always at least `1`, even when there are no items. |
| `totalItems` | `integer` | The total number of items across all pages after filters are applied. |
...
<Accordion>
<AccordionItem title="Page past the end">
...
<AccordionItem title="Invalid limit or page">
...
<AccordionItem title="Empty datasets">
...
<AccordionItem title="Filters and totals">
