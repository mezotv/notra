...
description: "How Notra receives GitHub webhooks, and what to use instead of outbound webhooks"
authors:
  - name: Dominik
    handle: dominik
...
<Callout type="warning">
  Notra does not send outbound webhooks to your servers. There is no way to register a callback URL, and Notra never signs or delivers events to third-party endpoints. To react to things that happen in Notra, poll the job endpoints or read the agent session stream. See [No outbound webhooks](#no-outbound-webhooks) below.
</Callout>
...
| --- | --- | --- |
| `400 Bad Request` | `error` | Invalid URL parameters, missing `X-GitHub-Event` or `X-Hub-Signature-256` header, no webhook secret generated for the repository yet, or a body that is not valid JSON in the expected shape. |
| `401 Unauthorized` | `error` | The signature does not match the repository's secret. Regenerate the secret in Notra and update GitHub if they drifted apart. |
| `403 Forbidden` | `error` | The integration is disabled, does not belong to the organization in the URL, or the repository does not belong to the integration. |
| `404 Not Found` | `error` | The integration or repository was deleted. |
| `500 Internal Server Error` | `error` | Notra could not finish processing the delivery. GitHub keeps the delivery in its history so you can redeliver it from the webhook settings page. |
| `501 Not Implemented` | `error` | The provider segment in the URL is not `github`. |
...
<CardGroup cols={2}>
  <Card title="Poll post generation" icon="refresh-cw" href="/api-reference/content/get-async-post-generation-status">
  ...
  <Card title="Stream agent sessions" icon="activity">
...
<CardGroup cols={2}>
  ...
  <Card title="Event triggers" icon="zap" href="/automation/event-based">
