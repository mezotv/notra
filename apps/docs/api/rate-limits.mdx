...
| `POST /v1/feedback/{organizationSlug}` | 30 requests / 10 minutes per IP address, plus 200 requests / hour per organization |
| `POST /v1/feedback` | 120 requests / minute per credential |
