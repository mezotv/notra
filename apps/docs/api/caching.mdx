...
<Note>
  When `status` is omitted the API returns only `published` posts. Pass `status=draft,published` explicitly if you want both, and treat "omitted" and "published" as the same cache entry.
</Note>
