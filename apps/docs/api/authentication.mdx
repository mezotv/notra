...
Every endpoint except `GET /v1/status` requires a bearer credential in the `Authorization` header. Two kinds of credential are accepted, and both are checked against the same [scopes](#scopes):
