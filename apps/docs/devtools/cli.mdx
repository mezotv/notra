...
For weekly schedules pass `--day-of-week` (0-6, Sunday is 0); for monthly schedules pass `--day-of-month` (1-31). `--output-type` is one of `changelog`, `blog_post`, `linkedin_post`, or `twitter_post`. `--repository` is repeatable. Add `--brand-voice <brandIdentityId>` to pin a brand identity and `--auto-publish` to publish changelogs and blog posts instead of saving drafts. Times are in UTC.
...
| `notra geo prompts` | `list`, `create --prompt <text>`, `update <promptId> --enabled` or `--no-enabled`, `delete <promptId>`, `import --config-file <json or csv>` |
| `notra geo sequences` | `list`, `create --name <name> --step <text>...`, `update <sequenceId>`, `run <sequenceId>`, `delete <sequenceId>` |
| `notra geo competitors` | `list`, `upsert --name <name> [--domain ...] [--kind direct or indirect]`, `suggestions --domain <domain>`, `import --config-file <file>`, `delete <name>` |
| `notra geo scans` | `start [--wait]`, `list [--page] [--limit]`, `get <scanId>` |
| `notra geo visibility` | `overview`, `timeseries`, `prompt-results`, `competitor-share`, `competitor <brand>`, `language-share` (all accept `--days` or `--from` and `--to`) |
| `notra geo gaps` | `list` |
| `notra geo briefs` | `list`, `get <briefId>`, `create --topic <text> [--auto-approve]`, `approve <briefId>` |
| `notra geo agent-readiness` | `get`, `scan` |
| `notra geo traffic` | `overview`, `log`, `journeys`, `journey <journeyId>`, `pages`, `setup`, `token`, `rotate-token` |
