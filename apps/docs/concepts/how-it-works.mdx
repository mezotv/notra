...
<Steps>
  ...
  <Step title="Scan">
    On every scan Notra sends each prompt to each enabled model, with web search enabled where the provider supports it, and keeps the full answer. A separate judge step checks every answer for mentions of your brand, your aliases and each tracked competitor.
  </Step>
...
<Tip>
  The tone you pick in Brand Identity selects a different prompt template (Conversational, Professional, Casual or Formal), so the same activity reads differently for each.
</Tip>
