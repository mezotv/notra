...
- **Brand sentiment**: a compact 0–100 score and daily trend beside the visibility metrics.
- **Mention rate by engine**: a table with **Engine**, **Mentions**, **Mention rate**, **Avg position**, **Sentiment**, **Last checked**, and **Trend** columns. Rows group into provider families; a family sheet shows the Search versus Without search split.
...
Daily buckets use UTC. The engine table combines model counts by engine family before
calculating its score, rather than averaging model scores. Days without classified
