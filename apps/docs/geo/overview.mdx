...
- **Brand sentiment**: positive, neutral and negative mentions, daily positive share, engine breakdowns and historical negative answers.
...
The main percentage is **positive mentions / classified mentions**. Classified mentions
have an exact positive, neutral or negative label and actually mention your brand.
For example, 3 positive, 2 neutral and 1 negative mention produce a 50% positive share.
Answers that do not mention the brand are counted separately and never enter this denominator.
Missing or unexpected labels on mentions are **unknown**, not neutral. Classification
coverage is classified mentions divided by all mentions. With no classified mentions,
the positive share is unavailable (shown as **—**), not zero.

Daily buckets use UTC. Counts are combined before calculating percentages, so engines
with different sample sizes are weighted by their saved answers. Days without classified
...
from unfinished scans may appear. Use the answer count and last measurement time for context.
...
Open **Negative answers** to load historical evidence, 25 answers at a time. Each entry
opens the exact saved answer and excerpt from that measurement, even if a later answer to
the same prompt is positive. Existing labels are read directly; viewing this card does not
run new AI calls or reclassify older answers.
