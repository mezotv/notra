...
const themeAtom: ReturnType<typeof atomWithHash<Theme>> = atomWithHash<Theme>(
  "theme",
  (() => {
    ...
    serialize(value) {
      return (
        Object.keys(THEMES).find(
          (k) => THEMES[k]?.name.toLowerCase() === value.name.toLowerCase()
        ) || ""
      );
    },
    deserialize(key) {
...
const darkModeAtom: ReturnType<typeof atomWithHash<boolean>> =
  atomWithHash<boolean>("darkMode", true);
