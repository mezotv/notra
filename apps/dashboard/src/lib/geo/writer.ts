...
export const getGeoContentBrief = Effect.fn("geo.writer.briefGet")(function* (
  ...
  const row = yield* requireBriefRow(organizationId, briefId);
...
export const approveAndStartGeoWriter = Effect.fn("geo.writer.start")(
  function* (
    ...
    if (!row) {
      const current = yield* requireBriefRow(organizationId, briefId);
    ...
    yield* Effect.tryPromise({
      try: () =>
        addActiveGeneration(organizationId, {
          runId,
          triggerId: GEO_WRITER_TRIGGER_ID,
      ...
      catch: (cause) => new GeoWriterStartError({ cause }),
    }).pipe(
      Effect.catch((error) => {
...
export const planGeoContentBrief = Effect.fn("geo.writer.plan")(function* (
  ...
  if (sourceKind !== "manual" && sourceId) {
    const open = yield* findReusableBrief(
      scope.projectId,
      sourceKind,
      sourceId
    );
  ...
  const generated = yield* Effect.tryPromise({
    try: async () => {
      try {
        const result = await generateGeoContentBrief({
          organizationId: scope.organizationId,
          input: {
            topic,
        ...
        await finalizeContentBilling({
          ...
          console.error("[GEO] planner credit release failed:", releaseError);
        });
        throw cause;
      }
    },
      ...
      new GeoWriterPlanError({
        message:
          cause instanceof Error ? cause.message : "Failed to plan the article",
        cause,
      }),
  });
  ...
  if (inserted.length === 0 && sourceKind !== "manual" && sourceId) {
    const winner = yield* findReusableBrief(
      scope.projectId,
      sourceKind,
      sourceId
    );
  ...
  const started = yield* approveAndStartGeoWriter(
    scope.organizationId,
    briefId,
    { autoApproved: true }
  );
