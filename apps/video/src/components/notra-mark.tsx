import { COLORS } from "../lib/theme";

interface NotraMarkSvgProps {
  size: number;
  drawProgress?: number;
}

const STROKE_LENGTH = 1100;

export function NotraMarkSvg({ size, drawProgress = 1 }: NotraMarkSvgProps) {
  return (
    <svg
      fill="none"
      height={size}
      viewBox="0 0 800 800"
      width={size}
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M572.881 462.223c-12.712 43.22-290.678 105.932-394.068 83.898l-48.305-10.169 48.305-78.814 68.644-104.237 73.729-106.78 251.695-127.119 78.814-22.881 17.796 17.796h10.17c17.796 35.593 3.945 147.458-12.712 195.763-25.424 73.729-124.576 96.61-177.966 114.407-4.064 1.355 96.61-5.085 83.898 38.136Z"
        fill={COLORS.lavender}
        opacity={drawProgress}
        stroke={COLORS.ink}
        strokeLinecap="round"
        strokeWidth={35}
      />
      <path
        d="M700 96.111c-162.712-4.237-510.508 111.356-600 607.627"
        stroke={COLORS.ink}
        strokeDasharray={STROKE_LENGTH}
        strokeDashoffset={STROKE_LENGTH * (1 - drawProgress)}
        strokeLinecap="round"
        strokeWidth={75}
      />
    </svg>
  );
}
