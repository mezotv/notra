...
- New workspace package `@notra/posthog` (`packages/posthog`): event catalog (`./events`), server client and capture helpers (`./server`), request-header context (`./request`), manual LLM capture helpers (`./llm`), AI SDK middleware wrapper (`./ai-sdk`). Consumed by `apps/dashboard`, `apps/api`, `apps/agent`, `apps/onboarding-agent`, `packages/ai`, `packages/geo-core`.
- LLM analytics: every routed model created through `gateway()` in `packages/ai/src/gateway.ts` is wrapped with `@posthog/ai/vercel` `withTracing` (privacy mode on for ZDR-required routes). Call sites pass `posthog: { feature, distinctId, projectId, sessionId }`. Eve agents capture `$ai_generation` / `$ai_trace` manually from their usage hooks. The OTel span-processor route was not used because `@contextcompany/otel` owns the OTel registration and does not accept additional processors.
...
- Events from sections 5.1 to 5.11 are wired on the client, in oRPC routers, route handlers, workflow steps, `apps/api` middleware/routes, and the Eve agent hooks. See the per-area helper modules under `apps/dashboard/src/lib/analytics/`, `apps/dashboard/src/constants/*-analytics.ts`, `apps/api/src/utils/analytics.ts`.
